...
def _rotate(P: Point, theta: float) -> Point:
    (x, y) = P
...
def create_texture(
    pattern: Tuple[List[Edge], Point, Point, int],
    h: int,
    k: int,
    lineWidth: float = 10,
    lineColor: Tuple[int, int, int] = (0, 0, 0),
    scale: float = 500,
) -> Image:
    ...
    theta = atan2(
        carth(B)[0], carth(B)[1]
    )  # Angle by which we need to rotate the image to get the right orientation
    # width and height of the final image
    width = int(_rotate(carth(C), theta)[0]) * scale
    height = int(_rotate(carth(B), theta)[1]) * scale
    ...
    carthEdges = []
    for P1, P2 in edges:
        carthEdges.append((_rotate(carth(P1), theta), _rotate(carth(P2), theta)))
