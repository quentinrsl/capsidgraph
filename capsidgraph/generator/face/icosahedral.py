...
def create_triangle(h:int,k:int,Tx:Vector2D,Ty:Vector2D)->Tuple[Point,Point,Point]:
	A = (0,0)
...
def is_in_triangle(P:Point,A:Point,B:Point,C:Point)->int:
	if is_under_line(P,A,B) == -1 or is_under_line(P,B,C) == -1 or is_under_line(P,C,A) == -1:
		return -1
	elif is_under_line(P,A,B) == 0 or is_under_line(P,B,C) == 0 or is_under_line(P,C,A) == 0:
		return 0
	else:
		return 1
...
def extract_triangle(edges:List[Edge],h:int,k:int,Tx:Vector2D,Ty:Vector2D)->List[Edge]:
	res = []
...
def create_face_edges(edges:List[Edge], Tx:Vector2D, Ty:Vector2D, h:int, k:int)->Tuple[List[Edge],Tuple[Point,Point,Point]]:
