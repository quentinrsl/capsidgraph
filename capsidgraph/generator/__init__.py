...
def create_icosahedral_capsid_graph(faceEdges:List[Edge],triangle_certices:Tuple[Point,Point,Point],bond_strength:List[float] | None=None)->nx.Graph:
	...
	#"Middle" row
	for i in range(5):
		G = merge_faces(G,coordinates, rotate_icosahedral_point,triangle_certices,2*i,2*i+1,0,False)
	for i in range(5):
		G = merge_faces(G,coordinates, rotate_icosahedral_point,triangle_certices,2*i+1,(2*i+2)%10,1,True)
	# "Top" and "Bottom" lines
	for i in range(10):
		G = merge_faces(G,coordinates, rotate_icosahedral_point,triangle_certices,i, 10+i, (i+1)%2, i%2==0)
	#link faces together
	for i in range(10):
		#Replace the following line with the commeted ones for an incomplete build of the graph
		G = merge_faces(G,coordinates, rotate_icosahedral_point,triangle_certices,10+i, 10 + ((i+2) % 10), 2, i%2==0)
	return G


def create_cubic_capsid_graph(face_edges:List[Edge],  square_vertices:Tuple[Point,Point,Point,Point], bond_strength:List[float]|None=None)->nx.Graph:
	if bond_strength != None and 0 in bond_strength:
		raise Exception("Bond cannot have 0 energy")
	G = nx.Graph()
	coordinates = {}
	...
	for face_id in range(6):
		face,face_coordinates = create_face_graph(face_edges,face_id,node_id,bond_strength=bond_strength)
		node_id += len(face.nodes)
		G = nx.compose(G,face)
		coordinates.update(face_coordinates)
